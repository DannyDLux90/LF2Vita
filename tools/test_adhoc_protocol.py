...
def source_contract_tests():
    ...
    assert "net70_champ_round_card" in ui
    assert 'draw_tournament_round("AdHoc' not in ui and "draw_team_tournament(view" not in ui
    ...
    # 8-slot lobby role contract: host + lowest occupied guest are active;
    # additional clients remain reserve and must not abort/launch the match.
    assert "active_guest_slot_internal" in lobby
    assert "g_peers[i].slot==active_slot" in lobby
    assert "reserve peer ended during match" in lobby
    assert "g_lobby.settings.status==LF2_NET_STATUS_WAITING" in lobby
    assert "net70_active_guest_slot" in ui and "RESERVE/ZUSCHAUER" in ui
    assert "l->local_slot==active_guest" in ui
    assert "present<2" in ui


def role_model_tests():
    def active_guest(present):
        return next((i for i in range(1, 8) if present[i]), -1)

    present = [True, True, True, True, False, False, False, False]
    assert active_guest(present) == 1
    # Reserve disconnect does not alter the active pair.
    present[3] = False
    assert active_guest(present) == 1
    # Active guest leaves: the lowest reserve is promoted deterministically.
    present[1] = False
    assert active_guest(present) == 2
    present[2] = False
    assert active_guest(present) == -1
