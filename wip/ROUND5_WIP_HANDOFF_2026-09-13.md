...
- Added authored `bg.dat` `zboundary` parsing and exposed the active top/bottom playable depth limits.
- Replaced the global `-70..75` fighter-depth clamp with per-background PC `zboundary` limits (`zboundary - GROUND_Y`) in movement/landing correction, loose-item placement and clone placement. This aligns fighter feet/depth travel with each stage's authored floor instead of using one Stanley-like approximation for every background.
...
- Hardware visual verification is still required after the new z-boundary/depth mapping.
...
- Hardware-check Stage 2/3 after camera reset, layer-order and zboundary fixes; use screenshots plus a fresh `lf2.log` if anything still appears vertically or horizontally displaced.
- Continue resource-formula tests: Dark HP/regeneration, John/Sorcerer/Jan healing, Milk/Beer, throw injury.
- Continue Frozen landing/break, John shield kind:9, state 3000/3006 reflect, held-weapon kind:5.
- Remaining raw `rand()` in active `fix3` is currently concentrated in visual FX and mode/setup randomness; simulation-affecting calls still need a final deterministic audit before declaring multiplayer PC-fidelity complete.
